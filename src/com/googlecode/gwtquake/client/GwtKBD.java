...
public class GwtKBD extends KBD {
  ...
  private static native boolean evtHasMovement(NativeEvent evt) /*-{
    return evt.movementX !== undefined;
  }-*/;

  private static native int evtMovementX(NativeEvent evt) /*-{
    return evt.movementX;
  }-*/;

  private static native int evtMovementY(NativeEvent evt) /*-{
    return evt.movementY;
  }-*/;
    ...
    public void onPreviewNativeEvent(NativePreviewEvent event) {
      ...
      if ("keydown".equals(nevt.getType())) {
        ...
        if ("mousemove".equals(nevt.getType())) {
          if (evtHasMovement(nevt)) {
            // *2 seems to work reasonably.
            mx = evtMovementX(nevt) * 2;
            my = evtMovementY(nevt) * 2;
          } else {
